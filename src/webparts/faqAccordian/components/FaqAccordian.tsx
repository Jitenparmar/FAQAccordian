...
import { IFaqAccordianProps } from './IFaqAccordianProps';
...
import { IFaqAccordianState } from "./IFaqAccordianState";
import IFaqListItem from "../models/IFaqListItem";
import { WebPartTitle } from "@pnp/spfx-controls-react/lib/WebPartTitle";
import './FaqAccordian.css';


export default class FaqAccordian extends React.Component<IFaqAccordianProps, IFaqAccordianState> {

  constructor(props:IFaqAccordianProps,state:IFaqAccordianState){
    super(props);
    this.state = {
  ...
  private listNotConfigured(props: IFaqAccordianProps): boolean {
  ...
  private readItems(): void {
    ...
    this.props.spHttpClient.get(restAPI, SPHttpClient.configurations.v1, {
      ...
      .then((response: SPHttpClientResponse): Promise<{ value: IFaqListItem[] }> => {
        return response.json();
      })
      .then((response: { value: IFaqListItem[] }): void => {
  ...
  public render(): React.ReactElement<IFaqAccordianProps> {
    ...
    const items: JSX.Element[] = this.state.items.map((item: IFaqListItem, i: number): JSX.Element => {
      return (
        <AccordionItem>
    ...
    return (
      <div className={styles.faqAccordian}>
        <div className={styles.container}>
          ...
          <div className='ms-Grid-row'>
            <div className='ms-Grid-col ms-u-lg12 pageButtons'>
              {pageButtons}
            </div>
          </div>
        </div>
